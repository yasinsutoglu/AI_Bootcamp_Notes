...
# GENERAL INTRO
###################################################
# Müşteriye Bir Ürünü Satın Aldıran Nedir?
# Social Proof (The wisdom of crowds => Rating)
# - Ürün puanları hesaplaması
...
df.head()
df.info() # değişkenler hakkında genel bilgi

df["Timestamp"] = pd.to_datetime(df["Timestamp"])

current_date = pd.to_datetime('2021-02-10 0:0:0')

# Verilen Rating üzerinden kaç gün geçmiş??
...
# zaman aralıklarına göre weight değerlerini de hesaba kattık
df.loc[df["days"] <= 30, "Rating"].mean() * 28/100 + \
    df.loc[(df["days"] > 30) & (df["days"] <= 90), "Rating"].mean() * 26/100 + \
    df.loc[(df["days"] > 90) & (df["days"] <= 180), "Rating"].mean() * 24/100 + \
    df.loc[(df["days"] > 180), "Rating"].mean() * 22/100

# Fonksiyonlaştıralım: (weight'lerin toplamı 100 olmalı dikkat!)
...
# 4.765025682267194 =>  virgülden sonra 5nci basamak bile önemli çok örneklemli sıralama için.

####################
# User-Based/User-Quality(Progress'e göre) Weighted Average
####################
